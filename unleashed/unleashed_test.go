...
var (
	...
	// client is the unleashed client being tested.
	client *Client
...
// setup sets up a test HTTP server along with a unleashed.Client that is
...
func setup() {
	...
	// unleashed client configured to use test server
	client = NewClient("", "")
...
func openTestFile(name, content string) (file *os.File, dir string, err error) {
	dir, err = ioutil.TempDir("", "go-unleashed")
	if err != nil {
		return nil, dir, err
	}
...
// If a nil body is passed to unleashed.NewRequest, make sure that nil is also
...
// function.  A redirect loop is pretty unlikely to occur within the Unleashed
...
func TestCheckResponse(t *testing.T) {
	res := &http.Response{
		...
		Body:       ioutil.NopCloser(strings.NewReader(`{"Description": "A value for Guid is required.", "DebugInformation": "Null" }`))}
	...
	want := &ErrorResponse{
		Response:         res,
		Description:      "A value for Guid is required.",
...
func TestSignatureCalculation(t *testing.T) {
	c := NewClient("5d13a26c-9aab-4791-98b2-014065e640c8", "sq7Uz7ChGPC2oC2ZCiFFIAvXix7rTjWBzSPKY7w69uPJoOmvIbZCiUAuB6pbAeGqkpym9347ALqShWX8j2CFkQ==")

	sign, err := c.CalcSignature("")
	if err != nil {
		t.Errorf("CalcSignature returned error: %+v", err)
	}
	expected := "f6tsD9YgjXRuAsKFe1NKMr3dKdB8hHsnUep35hz5ukU="
	if sign != expected {
		t.Errorf("Signature calculated incorrectly, expected %v, actual %v", expected, sign)
	}

	expected = "q6npNDkHLOyuRxAAinvUThJHJILA08j68E1J6g947Ek="
	sign, err = c.CalcSignature("a=b&c=d")
	if err != nil {
		t.Errorf("CalcSignature returned error: %+v", err)
	}
	if sign != expected {
	...
	c = NewClient("5d13a26c-9aab-4791-98b2-014065e640c8", "li0PveO71YKrvPg8SUBVJ0FNMqKL352LKi21bT8698CtCqyzg8cTUtbYAM2LSI49ghMSLXCnsexrAoAQoIQ==")

	expected = "aJAWxUbG7BpVcL1i4TPrEO/91QfP/2ryYv1yTIMlq9o="
	sign, err = c.CalcSignature("")
	if err != nil {
		t.Errorf("CalcSignature returned error: %+v", err)
	}
